...
class LaporanController extends Controller
{
    ...
    public function filterreport(Request $request)
    {
        ...
        ->whereBetween('orders.periode', [$start_date, $end_date])
            ...
            ->join('penitips', 'orders.id_penitip', '=', 'penitips.id')
            ->join('stocks', 'products.id_barang', '=', 'stocks.id_barang')
            ->select(DB::raw('sum(orders.jumlah) as jumlah'), DB::raw('sum(orders.total) as total'), DB::raw('sum(orders.laba) as laba'), DB::raw('sum(orders.uang_kembali) as uang_kembali'), DB::raw('sum(stocks.stok_awal) as stok_awal'), DB::raw('sum(stocks.stok_akhir) as stok_akhir'), 'orders.id_penitip', 'penitips.nama_penitip', 'products.nama_barang', 'products.harga_jual','orders.periode')
            ->orderBy('orders.invoice')
            ->groupBy('orders.id_barang')
            ->get();
        $total = Order::all()
        ...
        $kembali = Order::all()
            ...
            ->sum('uang_kembali');
        $penitip = sellers::all();
        return view('admin.laporan.report', compact('order', 'tanggalan', 'penitip', 'total', 'laba', 'kembali', 'start_date', 'end_date'));
    }
    ...
    public function report(Request $request)
    {
        ...
        $order = DB::table('orders')
            ...
            ->join('products', 'orders.id_barang', '=', 'products.id')
            ->join('penitips', 'orders.id_penitip', '=', 'penitips.id')
            ->select(DB::raw('sum(orders.jumlah) as jumlah'), DB::raw('sum(orders.total) as total'), DB::raw('sum(orders.laba) as laba'), DB::raw('sum(orders.uang_kembali) as uang_kembali'), 'orders.id_penitip', 'penitips.nama_penitip', 'products.nama_barang', 'products.stok_awal', 'products.stok_akhir', 'products.harga_jual', 'orders.periode')
            ->orderBy('orders.invoice')
            ->groupBy('orders.id_barang')
            ->get();
        $total = Order::all()
